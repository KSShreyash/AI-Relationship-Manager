...
- **Meeting creation**: calls Graph's calendar-write endpoint to create the event (with the chosen slot, the contact as an attendee when their email is known, and `isOnlineMeeting` set per the user's toggle) — this external call completes fully *before* any database transaction begins, the same hard guarantee AI Extraction's OpenAI call follows. Only once Graph confirms creation does a single local transaction run: insert the resulting event into `calendar_events` and set the new FK on the action item, mirroring how AI Extraction writes contacts + action items + `extracted_at` together in one transaction after its own external call returns.
